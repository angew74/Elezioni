...
public interface VotiListaDAO extends JpaRepository<VotiLista, Long> {
    ...
    @Modifying
    @Query("select new RicalcoloVotiLista(count(distinct idsezione) as numeroPervenute, s.municipio as municipio) from VotiLista v " +
            " inner join Sezione s on v.sezione.id=s.id" +
            " where v.tipoelezione.id=?1 " +
            " and s.municipio=?2 " +
            "group by s.municipio order by  s.municipio asc ")
    List<RicalcoloVotiLista> countPervenuteByMunicipio(int tipoelezioneid, Integer municipio);

    @Modifying
    @Query("select new RicalcoloVotiLista(count(distinct idsezione) as numeroPervenute) from VotiLista v " +
            " where v.tipoelezione.id=?1 ")
    List<RicalcoloVotiLista> countPervenute(int tipoelezioneid);

    @Modifying
    @Query("select new RicalcoloVotiLista(sum(f.votantitotali3) as numeroVoti, sum(i.iscrittitotaligen) as iscrittipervenute) from Affluenza f " +
    ...
    @Modifying
    @Query("select new RicalcoloVotiLista(sum(f.votantitotali3) as numeroVoti, sum(i.iscrittitotaligen) as iscrittipervenute, s.municipio as Municipio) " +
            " from Affluenza f " +
             " inner join Sezione s on f.sezione.id=s.id" +
    ...
    @Modifying
    @Query("select new RicalcoloVotiLista(sum(v.numerovoti) as numeroVoti,  l.id as idlista, l.denominazione as denominazione) from Lista l " +
    ...
    List<RicalcoloVotiLista> sumLista(Integer tipoelezioneid);


    @Modifying
    @Query("select new RicalcoloVotiLista(sum(v.numerovoti) as numeroVoti,  l.id as idlista, l.denominazione as denominazione, s.municipio as municipio) from Lista l " +
            " inner join VotiLista v on l.id=v.lista.id" +
            " and l.tipoelezione.id=v.tipoelezione.id "+
            " inner join Sezione s on v.sezione.id=s.id" +
            " where v.tipoelezione.id=?1 " +
    ...
    List<RicalcoloVotiLista> sumListaByMunicipio(Integer tipoelezioneid, int municipio);
    ...
    @Query("select new RicalcoloVotiLista(sum(v.numerovoti) as numeroVoti, l.id as idlista, l.denominazione as denominazione) from Lista l " +
            ...
            " and l.id=?2" +
            "group by l.id, l.denominazione ")
    List<RicalcoloVotiLista> sumListaByLista(Integer tipoelezioneid, Integer idlista);
    ...
    List<RicalcoloVotiLista> sumListaMunicipioByLista(Integer tipoelezioneid, Integer idlista);
