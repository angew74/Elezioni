...
public interface RicalcoloPreferenzeDAO  extends JpaRepository<RicalcoloPreferenze, Long> {
    ...
    List<RicalcoloPreferenze> findTopByTipoelezioneIdAndTiporicalcoloIdAndMunicipioNotInAnAndListaIdOrderByDataoperazioneDesc(int tipoElezioneId, int tipoRicalcoloId,int municipio, int idlista);
    @Modifying
    @Query("select a from RicalcoloPreferenze a " +
