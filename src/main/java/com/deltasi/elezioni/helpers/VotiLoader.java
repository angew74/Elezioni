...
public class VotiLoader {
    ...
    public HashSet<VotiSindaco> prepareVotiSindacoR(List<SindacoSimple> list, VotiGenerali vg) {
        HashSet<VotiSindaco> votiList = new HashSet<>();
        Integer tipoelezioneid = Integer.parseInt(env.getProperty("tipoelezioneid"));
        LocalDateTime oggi = LocalDateTime.now();
        String user = SecurityContextHolder.getContext().getAuthentication().getName();
        for (SindacoSimple l : list) {
            VotiSindaco v = votiSindacoService.findBySindacoIdAndSezioneNumerosezioneAndTipoelezioneId(l.getId(), l.getNumerosezione(), tipoelezioneid);
            v.setDataoperazione(oggi);
            v.setNumerovoti(l.getVoti());
            v.setNumerovotisolosindaco(l.getSolosindaco());
            v.setUtenteoperazione(user);
    ...
    public HashSet<VotiLista> prepareVotiLista(List<SindacoSimple> sindaci, VotiGenerali vg) {
        ...
        for (SindacoSimple s : sindaci) {
            ...
            for (ListaSemplice u : r.getListe()) {
                VotiLista v = new VotiLista();
                v.setDataoperazione(oggi);
                v.setNumerovoti(u.getVoti());
                v.setUtenteoperazione(user);
                v.setSezione(sezione);
                v.setTipoelezione(tipoElezione);
                Lista lista = listaService.findById(u.getIdlista());
                v.setLista(lista);
                v.setVotiGenerali(vg);
                l.add(v);
            }
    ...
    public HashSet<VotiLista> prepareVotiListaR(List<SindacoSimple> sindaci, VotiGenerali vg) {
        HashSet<VotiLista> l = new HashSet<VotiLista>();
        LocalDateTime oggi = LocalDateTime.now();
        Integer tipoelezioneid = Integer.parseInt(env.getProperty("tipoelezioneid"));
        VotiLista v = new VotiLista();
        ...
        for (SindacoSimple s : sindaci) {
            ...
            if (r == null) {
           //     List<Lista> ll = listaService.findBySindacoIdAndTipoelezioneId(s.getIdsindaco(),tipoelezioneid);
            } else {
                v = votiListaService.findById(r.getListe().get(0).getId());
                v.setDataoperazione(oggi);
